...
export default class Datasync {
  ...
  private resolveRange(interval: Interval): number[][] {
    ...
    for (let i = 0; i < dataPoints / numCandlesInRequest; i++) {
      const s =
        this.start + IntervalDuration[interval] * numCandlesInRequest * i;
      const f = Math.min(
        this.start + IntervalDuration[interval] * numCandlesInRequest * (i + 1),
        now
      );
      range.push([s, f]);
    }
  ...
  async fetchData(asset: string, interval: Interval) {
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      const tempfile = "./temp/" + destination + "-part-" + i;
      ...
      fs.writeFileSync(tempfile, JSON.stringify(r.data), "utf8");
      requests.push(r);
      await sleep(500);
    }
